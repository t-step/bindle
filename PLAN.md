...
3. The durable work ledger is implemented and verified but not yet adopted as standing policy. `specs/001-durable-work-ledger/`, `src/bindle/work_ledger.py`, and `tests/test_work_ledger.py` now establish a Bindle-owned SQLite coordination ledger plus a coordinator-agnostic disposable projection. SQLite here belongs to Bindle, not Symphony. The prior Symphony exploration plan is archived as historical evidence because its older "Symphony SQLite tracker" and "manual SQLite INSERT" framing is superseded. Current coordination framing is `plans/active/2026-08-26-coordination-current-framing.md`: adopt the ledger first, then re-verify the pinned Symphony fork, define the narrow projection-to-Symphony mapping, and prove one end-to-end execution path before adding any Bindle Symphony lifecycle surface.
