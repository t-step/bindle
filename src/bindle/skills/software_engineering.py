...
Source of truth is `t-step/skills` (`main`), never vendored into Bindle (D035):
one marketplace (`t-step-skills`) publishing one plugin whose
`plugins/software-engineering/skills/` holds plain, portable `SKILL.md`
directories.

Claude Code uses its native marketplace/plugin mechanism (`claude plugin
marketplace add`, `claude plugin install|uninstall --scope project`).
Marketplace registration is machine-global regardless of `--scope`, so `add()`
registers it if missing but `remove()` never unregisters it (D032/D033: a
repository-scoped command must not mutate machine-global state). `remove()`
fails loudly if `.claude/settings.json` shows the plugin enabled but the
`claude` CLI is unavailable, rather than leaving stale configuration while
reporting nothing to remove.

Codex has no package manager or CLI lifecycle for skills (as of 0.146.0); the
native mechanism is a plain `.agents/skills/<name>/SKILL.md` directory. `add()`
materializes each skill verbatim from a fresh shallow clone: a point-in-time
snapshot, not a live sync.

Ownership is worktree-scoped. The marker lives at
`<git-dir>/bindle-skills/software-engineering.codex.json` (`repo_info.git_dir`,
NOT `git_common_dir`), so each linked worktree gets independent evidence that
Git removes with the worktree, and one worktree's `remove()` cannot corrupt
another's.

`info/exclude` is shared repository state, so `_reconcile_exclude_block()`
recomputes the required ignore lines from every live worktree's marker (walking
`<git-common-dir>/worktrees/*`) and rewrites exactly one delimited Bindle-owned
block, leaving every other line untouched.

Each marker entry pairs a skill name with a content digest (`_digest_dir`).
`remove()` deletes only directories that still match; anything modified or
foreign is reported as a conflict, and its ownership evidence is retained so a
later `remove()` can still act on it.
"""
...
def _marker_path(repo_info: RepoInfo) -> str:
    # git_dir, not git_common_dir: Git removes it with the worktree.
...
def _all_worktree_git_dirs(repo_info: RepoInfo) -> list[str]:
    # Main git_dir is git_common_dir; walk worktrees/ (not `git worktree list`): dirs may be gone.
...
def _digest_dir(path: str) -> str:
    """Deterministic digest of every file's path and content under `path`.

    Changes on any add, remove, rename, or edit, so `remove()` can detect
...
def _reconcile_exclude_block(repo_info: RepoInfo) -> None:
    """Rewrite Bindle's info/exclude block to the union of all worktree markers.

    Never touches lines outside the block; a pre-existing identical entry is
    left alone, not duplicated. Lines already in the block are kept while any
    worktree requires them, without the tracked/ignored check (self-referential:
    the block itself makes `git check-ignore` say yes).
    """
    ...
    if not new_lines:
        # Clear rather than unlink (Git infrastructure), leaving no stale block.
...
def _codex_add(repo_info: RepoInfo) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="bindle-skills-clone-") as tmp:
        ...
        # New names, or owned ones missing on disk, need (re)materializing.
        to_write = [
            name
...
def _codex_remove(repo_info: RepoInfo) -> tuple[bool, str]:
    ...
    for name, digest in skills.items():
        ...
        if _digest_dir(path) == digest:
            ...
            # Modified: keep it and its ownership evidence for a later remove.
