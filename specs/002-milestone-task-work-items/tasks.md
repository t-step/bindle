...
- [ ] T016 [US5] Extend `archive_work_item()` with the milestone precondition from `data-model.md` ("Archival"): refuse (return `False`) when `type='milestone'` and any child (`parent_id` naming it) has a status outside `('done', 'superseded')` (a child is always a `task`, which per FR-005 can never itself be `accepted`). This precondition MUST be embedded directly in the guarded archival `UPDATE`'s own `WHERE` clause, inside the same `BEGIN IMMEDIATE` transaction as the mutation — not evaluated by a separate `SELECT` before the transaction opens, per FR-015's atomicity requirement (mirroring `mark_in_review`'s own FR-010 inline-precondition pattern); confirm the existing archival transaction's surviving-columns set now also preserves `type` for a milestone's thinned row, in `src/bindle/work_ledger.py`. Depends on: T002, T004.
- [ ] T016a [US5] Extend `archive_work_item()` with the task-parent-lifecycle precondition from `data-model.md` ("Archival", FR-015a): refuse (return `False`) when the item being archived is a `task` naming a `parent_id` whose milestone `status` is currently `open` or `review` — archiving a task deletes that task's own evidence rows, which can otherwise invalidate the parent's review-readiness (or the evidence behind an already-in-flight review) without the parent's row recording anything changed. Permitted once the parent is `accepted`/`superseded`, or unconditionally when the task has no `parent_id` (001's original behavior, unchanged). MUST be embedded as a correlated `EXISTS` directly in the same guarded `UPDATE`'s `WHERE` clause as T016, inside the same `BEGIN IMMEDIATE` transaction — not a separate pre-check. Requires aliasing the `UPDATE` target (`UPDATE work_items AS wi ...`) so the correlated subquery's own `work_items` scope does not shadow the outer row's `parent_id` (see `research.md`'s "Decision: task archival precondition on parent milestone lifecycle" for the empirically-confirmed pitfall), in `src/bindle/work_ledger.py`. Depends on: T002, T016.
...
- [ ] T017a [US5] `TestTaskArchivalParentLifecycle` (FR-015a): an attributed, done+evidenced task cannot be archived while its parent milestone is `open`; cannot be archived while the parent is `review`; can be archived once the parent is `accepted`; can be archived once the parent is `superseded`; an unattributed task retains 001's original, parent-independent archival behavior; a failed archival attempt leaves the task's row and evidence completely unmodified; two deterministic two-connection concurrency tests (mirroring T017's own `TestMilestoneArchival` race-construction technique) force each valid ordering of a concurrent `accept_milestone(parent)` vs. `archive_work_item(child)` race in turn and confirm archival never succeeds against a parent still `open`/`review`, in `tests/test_work_ledger.py`. Depends on: T016a.
