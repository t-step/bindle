...
**Decision**: `archive_work_item` gains a precondition when `type = 'milestone'`: refuse (return `False`, matching 001's existing no-op-on-guard-failure convention) if any child task's status is not in `('done', 'accepted', 'superseded')`. Task archival is otherwise unchanged from 001.
