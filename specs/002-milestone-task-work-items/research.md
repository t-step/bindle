...
**Decision**: `archive_work_item` gains a precondition when `type = 'milestone'`: refuse (return `False`, matching 001's existing no-op-on-guard-failure convention) if any child task's status is not in `('done', 'superseded')` — a child is always a `task`, which per FR-005 can never itself be `accepted` (a milestone-only status), so that value is not listed. Task archival is otherwise unchanged from 001. This precondition is embedded directly in the guarded archival `UPDATE`'s own `WHERE` clause, inside the same `BEGIN IMMEDIATE` transaction as the mutation — not evaluated by a separate `SELECT` before the transaction opens.
...
## Decision: task archival precondition on parent milestone lifecycle

**Decision** (FR-015a): `archive_work_item` gains a second, independent precondition: refuse (return `False`) when the item being archived is a `task` naming a `parent_id`, and that parent milestone's `status` is currently `open` or `review`. Archival remains permitted once the parent has reached `accepted` or `superseded`, and remains permitted unconditionally for a task with no `parent_id` at all — this precondition adds nothing to 001's original, unattributed-task archival path. Embedded the same way as the milestone precondition above: a correlated `EXISTS` directly in the guarded `UPDATE`'s own `WHERE` clause, inside the same `BEGIN IMMEDIATE` transaction as the mutation.

**Rationale**: this is the *reverse* direction of the problem the milestone precondition above already solves. That precondition stops a milestone from being archived while a child is still unresolved; it says nothing about what happens when a *resolved* child is archived out from under a milestone that has not yet finished (or even started) reviewing it. Archival deletes the archived item's own evidence rows (`DELETE FROM work_item_evidence`, unchanged from 001) — and `is_review_ready` (FR-009) is computed from each child's current status *and* its recorded evidence. Left unguarded, this sequence is reachable through the ordinary public API alone, no raw SQL required:

```
M open, T done+evidenced -> M is_review_ready() == True
archive_work_item(T)      -> T's evidence rows deleted; T itself stays "done"
                           -> M is_review_ready() == False (T no longer carries qualifying evidence)
```

or, worse, after a milestone has already entered review on the strength of that evidence:

```
mark_in_review(M)         -> M enters review, review-ready at that moment
archive_work_item(T)      -> the evidence that justified review is now gone
accept_milestone(M)       -> accept_milestone does not recompute readiness
                              (research.md/spec.md: acceptance is deliberately
                              a single guarded transition, not a second
                              readiness check — see "Decision: 'review
                              readiness' — defined predicate" above), so
                              M is accepted with its justifying evidence
                              already destroyed underneath it
```

Neither case corrupts referential integrity (`T`'s `id`/`type`/`status` all survive archival unchanged, per FR-016) — the problem is purely that the *evidence trail* a human review decision depended on can be silently pulled out from underneath that decision, while the milestone's own row records nothing about it having happened. Refusing the archival while the parent is still `open`/`review` is the minimal fix: it does not touch `is_review_ready`'s definition, does not make `accept_milestone` recompute readiness, and does not add any new stored flag — it simply defers the child's own archival (a purely disposable/cache-thinning operation, per `docs/DATA-OWNERSHIP.md`'s "capture requires a reason" posture applied to retention as much as capture) until the parent's review boundary has closed and its child set can never change again.

Embedding this as a pre-transaction, separate-statement check was rejected for the same reason the milestone precondition's own separate-check draft was rejected (see above): it would leave a race window between "confirmed parent is accepted/superseded" and "archived" in which a concurrent `mark_in_review`/`decline_review`/`accept_milestone` on the parent could land in between. Embedding the condition inline, inside the same `BEGIN IMMEDIATE` transaction, closes that window: SQLite permits only one writer at a time (`BEGIN IMMEDIATE` acquires the write lock for the whole transaction; `accept_milestone`'s own single guarded `UPDATE` is already atomic on its own under `connect()`'s autocommit mode), so a concurrent `accept_milestone(parent)` and `archive_work_item(child)` always serialize to one of exactly two valid outcomes — archival's transaction observes the parent as still `review` and refuses (acceptance then proceeds afterward), or acceptance commits first and archival's subsequent transaction observes the already-`accepted` parent and succeeds. Archival succeeding against a parent that remains `open` or `review` is not a reachable outcome of either ordering. Verified with a deterministic two-connection test forcing each ordering in turn (manual `BEGIN IMMEDIATE`/`COMMIT` control on a second connection, mirroring the existing `test_archival_precondition_is_atomic_against_a_concurrent_child_insertion` regression's own technique), not left to thread-scheduling luck.

A subtle implementation pitfall surfaced during this fix and is worth recording: the parent-lifecycle precondition's `EXISTS` subquery (`SELECT 1 FROM work_items p WHERE p.id = ... AND p.status IN ('accepted', 'superseded')`) must reference the *outer* row's own `parent_id` — but the subquery's own `FROM work_items p` clause introduces a second `work_items` scope that also has a `parent_id` column. An unqualified `parent_id` reference inside that subquery resolves to the inner table (`p.parent_id`), not the outer row being updated, per ordinary SQL name-resolution shadowing rules — silently matching nothing, not raising an error. The guarded `UPDATE` therefore aliases its own target (`UPDATE work_items AS wi ...`) and qualifies every outer reference (`wi.parent_id`, `wi.id`, `wi.status`, `wi.archived_at`) so the correlation is unambiguous. Confirmed empirically: the unaliased form parses and runs without error but never matches a task whose parent is genuinely `accepted`/`superseded` — a silent false-negative, not a crash, making it easy to miss without a passing "task can be archived after parent accepted" test case.

**Alternatives considered**: Making `accept_milestone` recompute and re-verify readiness at the moment of acceptance (closing the second scenario above without touching archival at all). Rejected: this feature's spec and this same research document (see "Decision: 'review readiness' — defined predicate...") already establish that acceptance is a deliberate, single guarded human-judgment transition, not a second automated readiness gate — re-deriving readiness inside `accept_milestone` would blur that boundary and does not address the *first* scenario above (evidence disappearing while the milestone is still merely `open`, never having entered review at all). Refusing the archival at its source closes both scenarios with one precondition, in the one place (`archive_work_item`) that actually mutates the evidence the readiness computation depends on.
