...
- What happens when an attributed, `done`-and-evidenced child task is archived while its parent milestone is still `open` or `review` — is this the same "unresolved child" case as above? No — the task itself is resolved; the problem is the reverse direction: archival deletes that task's own evidence rows, which can invalidate the parent's review-readiness (or the evidence behind an already-in-flight review) without ever touching the parent's own row. FR-015a MUST refuse this archival while the parent is `open`/`review`, permitting it only once the parent reaches `accepted`/`superseded` (its child set can never change again after that point).
...
- **FR-015**: The model MUST refuse to archive a milestone work item (extending 001's existing archival operation) while any of its child tasks has a status other than `done` or `superseded` — a child is always a `task`, which per FR-005 can never itself be `accepted` (that is a milestone-only status) — archival of a milestone with unresolved children is rejected outright, not silently allowed to orphan live children's attribution. This precondition and the archival mutation itself MUST be atomic with respect to a concurrent writer: a separate pre-check followed by a later, separate archival mutation would leave a race window in which an unresolved child could be added (or a resolved one reopened) in between, the same class of check-then-act race FR-010 already closes for `mark_in_review`.
- **FR-015a**: The model MUST refuse to archive a `task` work item that has a `parent_id` while its parent milestone's `status` is `open` or `review`; archival of such a task MUST be permitted once the parent milestone reaches `accepted` or `superseded`, and MUST remain permitted unconditionally (unchanged from 001) for a task with no `parent_id` at all. Rationale: archiving a task deletes that task's own evidence rows (FR-016/data-model.md's "Archival"), so archiving an attributed, resolved task while its parent is still `open` or `review` can silently invalidate the parent's review-readiness (FR-009) — or the very evidence that justified an already-in-flight review — out from underneath it, without the parent's own state ever recording that anything changed. This precondition and the archival mutation itself MUST be atomic with respect to a concurrent parent-milestone lifecycle transition (`mark_in_review`, `decline_review`, or `accept_milestone`) — a separate pre-check followed by a later, separate archival mutation would leave a race window in which the parent could transition in between, the same class of check-then-act race FR-010 already closes for `mark_in_review` and FR-015 already closes for a milestone's own unresolved-children check.
