...
- **FR-015**: The model MUST refuse to archive a milestone work item (extending 001's existing archival operation) while any of its child tasks has a status other than `done`, `accepted`, or `superseded` — archival of a milestone with unresolved children is rejected outright, not silently allowed to orphan live children's attribution.
