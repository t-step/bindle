...
The status transition and the rationale-locator recording are **two separately committed operations**, not one transaction (spec.md FR-010a; `research.md`'s "Partial-failure semantics"). When `--evidence` is supplied, the transition succeeds, and the subsequent `add_evidence()` call also succeeds, exactly one evidence pointer is recorded against the milestone (`kind='other'`, `value=<locator>`, `note=<text or None>`) — this is the only case the "exactly one evidence pointer" guarantee covers. If the transition succeeds but `add_evidence()` then fails, the transition remains committed (never rolled back), `DecisionResult.rationale_error` (`data-model.md`) carries the failure distinctly from `reason` (so it is never confused with `not_in_review`/`not_found`/`not_a_milestone`), and the CLI reports both facts — the decision succeeded, the locator did not record — rather than treating the whole command as a rejection. The correct remedy for a failed rationale recording is to record it separately afterward (e.g. a direct `add_evidence()` call); the caller MUST NOT re-invoke `accept`/`decline` against the same milestone merely to retry the rationale recording, since the milestone has already left `review` and the retried command would either no-op or be misread as a second decision attempt.
...
- **CLI exit codes**: `0` on success; `1` on any rejection (not found, not a milestone, not ready, not in review, already claimed) — consistent with `bindle`'s existing exit-code convention (`_cmd_work_claim` et al.), with the specific reason always printed to stderr as `bindle milestone <verb>: ...`. `accept`/`decline` exit `0` whenever the status transition itself succeeded, even if the optional rationale-locator recording subsequently failed (`DecisionResult.rationale_error`) — the decision the caller asked for did succeed, and an automation checking exit code alone must not read a nonzero code as license to retry the decision. When `rationale_error` is set, the CLI additionally prints a distinct warning line to stderr naming the failure, separate from the success line, so a human reading the output notices even though the exit code is `0`.
