...
- **`bindle work` and the new milestone surface remain two deliberately separate CLI command groups**, not variants of the same subcommands — this is what makes User Story 5's "neither surface can perform the other's mutation" true by construction (each group's own type-guard) rather than by convention alone, mirroring `specs/003`'s own reasoning for rejecting milestones on the task-facing surface (`task-write-surface.md`: "categorically rejected... never silently treated as a task").
