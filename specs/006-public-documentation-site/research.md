...
- **Pure Python, zero new-language toolchain.** MkDocs is a Python package installable as an ordinary `uv`-managed dependency, consistent with this repository's Python/`uv` baseline (`pyproject.toml`, `AGENTS.md` CLI invocation). It introduces no Node.js/npm, no Rust toolchain, and no second package manager. It is repository documentation tooling, not a Bindle runtime dependency — declared under `[dependency-groups]` (`dev`, PEP 735) rather than `[project].dependencies`, corrected 2026-08-29 during PR review; `uv run` includes the `dev` group by default, so `uv run mkdocs build --strict` (`scripts/check.sh`) needs no separate `--group` flag or second environment.
...
**Decision** (revised 2026-08-29 by explicit human decision, superseding this decision's original "link out" recommendation; corrected 2026-08-29 to move Home's source to `docs/index.md` — see "Correction" below): site-only orientation pages live at **`docs/site/`** — `docs/site/getting-started.md`, `docs/site/how-bindle-works.md` — with the sole, narrow exception of Home itself at **`docs/index.md`**, the conventional MkDocs root-index source — and every existing canonical `docs/*.md` reference document is **rendered directly by MkDocs**, in place, as a real site page. The governing rule, stated by the operator: *one canonical Markdown source, rendered through the documentation site.* Home and the other `docs/site/` pages orient and route readers into the canonical documents; they do not copy or restate their substantive content (FR-008 still governs the *new* pages, not whether the existing files are built).
...
**Correction (2026-08-29, PR review pass)**: this decision's original text placed Home at `docs/site/index.md`, and the "Implementation-time nuance" paragraph below then correctly predicted that this choice leaves the site's bare root unreachable without a redirect — but the shipped `mkdocs.yml` took the "accept nav-only entry, no root index.html" branch of that nuance instead of the `docs/index.md` branch, which is not acceptable for a static site whose primary entry point is Home. The fix, applied in this same pass: Home's source moves to `docs/index.md` — MkDocs's own ordinary root-index convention, requiring no redirect, no custom JavaScript, and no server behavior — which is exactly option (a) this decision's "Implementation-time nuance" paragraph already named. `docs/site/` otherwise keeps every other site-only page; this is a narrow mechanical exception for the root index, not a reopening of the information architecture.
...
**Implementation-time nuance, resolved (not a further planning decision)**: MkDocs maps output paths to source paths relative to `docs_dir`. Only a page actually named `docs/index.md` builds to the site root's own `index.html`; a page at `docs/site/index.md` instead builds to `site/index.html`, leaving the bare root without a usable landing page. Resolved by moving Home's source to `docs/index.md` (see "Correction" above) — the conventional MkDocs root-index shape, not a redirect stub and not nav-only entry. Verified: `uv run mkdocs build --strict --site-dir <dir>` produces `<dir>/index.html` as the Home page, with working links to Getting Started, How Bindle Works, and the Reference documents.
...
- `docs/index.md` (Home — MkDocs's own root-index convention, the one narrow exception to `docs/site/`), `docs/site/getting-started.md`, `docs/site/how-bindle-works.md` — decided location and shape (human decision, 2026-08-29; Home's exact source path corrected 2026-08-29, see Decision 2's "Correction"); no separate `reference/index.md` is authored (Decision 2)
- A new `scripts/check.sh` section (edit, not a new file)
- A gitignore entry for the build output directory (edit to `.gitignore`)

**Files expected to be modified**: `README.md`, `docs/SCOPE.md`, `docs/DATA-OWNERSHIP.md`, `docs/TOOLCHAIN.md`, `scripts/check.sh`, `.gitignore`, `pyproject.toml` (MkDocs declared under `[dependency-groups]`/`dev`, not `[project].dependencies` — Decision 1 correction), `uv.lock` (regenerated via `uv lock`, never hand-edited).
