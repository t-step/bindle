...
- **The durable work ledger is real, tested, and load-bearing, and README.md never mentions it.** `bindle init` (verified by direct execution against a disposable local repository) unconditionally provisions `.bindle-work/ledger.sqlite3` and `.bindle-work/symphony-projection.sqlite3` (D043), and `bindle work load-speckit`, `bindle work status`, `bindle work status --json`, and `bindle work forecast` (verified by direct execution against `specs/005-work-state-visibility/tasks.md` loaded into a fresh ledger) correctly report dispatchable and blocked work-item state and a dependency frontier with no Symphony process involved at any point. (Claim/release/done were not exercised by this direct-execution pass — they are separately implemented and tested, per `specs/003-symphony-task-integration/` and D041's Symphony-side proof, but this Grounding entry only asserts what was directly observed here.) Yet README.md's CLI section documents only `bindle --version`, `bindle repo info`, `bindle branch`, and the lifecycle-command table (`init`/`remove`/`status`/etc.) — it names no `bindle work` or `bindle milestone` subcommand anywhere, even though `specs/001`–`005` (adopted in D038, D039, D042, D045) are the majority of this repository's actually-implemented, actually-tested product surface.
...
A newcomer follows a documented getting-started path and, using only real `bindle` commands against a disposable local repository, provisions Bindle, loads a Spec Kit feature's tasks into the ledger, and observes meaningful dispatchable/blocked work state and a dependency forecast — without installing, configuring, or running Symphony.
...
3. **Given** the documented getting-started path, **When** a newcomer completes it, **Then** they have observed dispatchable and blocked work-item state and a dependency forecast without installing, configuring, or starting Symphony at any point.
...
- **SC-003**: Zero implemented capabilities (`bindle work status`/`forecast`, `bindle milestone` review lifecycle, the Spec Kit loader, the Symphony projection) are described as future, planned, or unimplemented anywhere in the current newcomer-facing or current-reference documentation this feature authors or edits (README.md, the documentation site's own pages, and any touched `docs/*.md` file) — verified by review against FR-002. This criterion explicitly excludes material that is clearly historical, such as `docs/DECISIONS.md`, where an entry accurately describing a capability as future or unimplemented *as of the date that entry was written* is correct historical record, not a violation, per FR-009.
...
- **SC-005**: Every decision-log entry present in `docs/DECISIONS.md` before this feature's implementation remains byte-identical, unedited, and in the same order after implementation — verified by diff restricted to that pre-existing content; no existing entry is edited, reordered, or removed. A new, appended entry recording this feature's own adoption, per this repository's normal decision-log convention, is permitted and does not violate this criterion.
