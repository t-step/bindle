...
- `docs/index.md` — new, Home; MkDocs's own root-index convention, the one narrow exception to `docs/site/` (corrected 2026-08-29, PR review — see research.md Decision 2's "Correction")
- `docs/site/` — new, nested inside the existing `docs/` directory (getting-started.md, how-bindle-works.md)
...
- `pyproject.toml`, `uv.lock` — existing, edited (MkDocs declared under `[dependency-groups]` `dev`, not `[project].dependencies` — corrected 2026-08-29, PR review)
...
- [x] T008 [US4] Create `mkdocs.yml` at the repository root: leave `docs_dir` at its MkDocs default (resolves to the existing `docs/` directory, covering both `docs/site/*.md` and every existing canonical `docs/*.md` file with no override — research.md Decision 2/3); `nav:` = Home → `index.md`, Getting Started → `site/getting-started.md`, How Bindle Works → `site/how-bindle-works.md`, and a "Reference" nav section grouping `PHILOSOPHY.md`, `SCOPE.md`, `DATA-OWNERSHIP.md`, `SYMPHONY.md`, `WORKTREES.md`, `PRIVACY.md`, `TOOLCHAIN.md`, and `DECISIONS.md` (each rendered directly, per the human decision — no separate reference/pointer page is authored); `theme:` MkDocs's own built-in default only, no third-party theme, no custom CSS/JS (human decision, 2026-08-29); `plugins:` the bundled `search` plugin only. Resolve the "site root reaches the Home page" nuance research.md Decision 2 flags by putting Home's source at `docs/index.md` — the conventional MkDocs root-index shape — so the built site's bare root has a real, usable `index.html`; this is a mechanical detail, not a new planning decision. **Corrected 2026-08-29 (PR review)**: the initial implementation instead put Home at `docs/site/index.md` (nav: `Home: site/index.md`) and left the site root without an `index.html`, contradicting FR-007's "reachable from a landing page" intent for a deployable site; verified fixed by `uv run mkdocs build --strict --site-dir <dir>` producing `<dir>/index.html`.
- [x] T009 [US4] Author `docs/index.md` (Home): brief orientation linking to Getting Started, How Bindle Works, and the Reference section — no restatement of any canonical document's substantive content (FR-008). **Corrected 2026-08-29 (PR review)**: file moved from `docs/site/index.md` to `docs/index.md` (T008's correction); its own relative links updated accordingly (`site/getting-started.md`, `site/how-bindle-works.md`, and the Reference documents without a leading `../`).
...
- [x] T018 Content-correctness reconciliation review: re-read every new/edited page and section from Phases 3–6 (`README.md`, `docs/SCOPE.md`, `docs/DATA-OWNERSHIP.md`, `docs/TOOLCHAIN.md`, `docs/index.md`, `docs/site/getting-started.md`, `docs/site/how-bindle-works.md`) against spec.md's Grounding section and `docs/DECISIONS.md` D038/D039/D041/D042/D045/D046; confirm zero implemented capabilities are described as future/placeholder anywhere in this newly-authored or edited material (FR-002, SC-003), confirm no contradiction with any of the six named decisions (SC-007), and confirm the full ownership/architecture explanation is reachable within one navigation step from Home while `README.md` no longer carries it as primary content (SC-008, direct before/after comparison). **Re-run 2026-08-29 (PR review correction pass)** against the corrected `README.md` (its CLI section's `bindle work`/`bindle milestone` coordination surface and Current Focus section no longer describe implemented capabilities as future) and the corrected `docs/index.md`/`mkdocs.yml` (Home at `docs/index.md`) — no new gap found.
  - *Depends on*: T002, T005, T006, T009, T010.
- [x] T019 [P] Structural-safety review: inspect every file this feature adds or edits (`mkdocs.yml`, `docs/index.md`, `docs/site/*.md`, the `scripts/check.sh` addition, `pyproject.toml`) and confirm none of them contains, imports, or invokes a server/backend process, an authentication mechanism, or any code path that opens, queries, or otherwise reads a `.bindle-work/*.sqlite3` file (FR-013, FR-014, SC-006) — i.e., this feature builds nothing `docs/DECISIONS.md` D045 already declined under a different name.
