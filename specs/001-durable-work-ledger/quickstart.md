...
3. **New — explicit recovery**: with the `stale_claim` finding from step 2 in hand, an agent deletes `WI-1`'s Claim Record on that evidence — this is the override *release*. `WI-1` is now unclaimed and claimable by anyone; the releasing agent does not thereby own it. The agent then attempts to acquire a new claim for `WI-1` through the ordinary exclusive create-if-absent operation (data-model.md, "Claim atomicity contract"), exactly like any other acquisition. If nothing else races it, this succeeds and `WI-1` is claimed by the new owner. If another actor's acquire attempt wins the race in the gap between the release and the new create — a real, observable possibility this model does not exclude — the releasing agent's own create instead fails with the ordinary "already claimed" result, and it must not assume it owns `WI-1`; the other actor does. Release and reacquisition remain two independent operations, each individually (not jointly) arbitrated by the same exclusive-create guarantee that governs any other claim — no atomic delete-and-replace primitive is introduced. ✅ traces to FR-019, data-model.md "Staleness," and the spec.md Edge Case on override release.
