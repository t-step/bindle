...
- **2026-08-26 revalidation** (persistence-model correction: SQLite replaces plain-per-item-TOML-files): re-checked after `research.md`/`data-model.md`/`contracts/`/`plan.md`/`quickstart.md` were revised to specify a SQLite-backed ledger instead of one plain-text file per work item, per an intentional, directed override (not a reopened storage-format evaluation) recorded in `research.md`'s "Decision: storage format." All checklist items still pass, and the item-35 note above remains accurate without amendment: spec.md itself named neither TOML nor SQLite before this correction and names neither now — the storage-technology choice stayed confined to research.md/data-model.md/contracts/plan.md throughout, exactly as this checklist's own discipline requires. Two spec.md passages needed wording adjustments to stay mechanism-neutral rather than implicitly assuming a multi-step, non-atomic write process as the routine cause of a corrupt claim (the crash Edge Case and SC-010) — both were already abstract in intent; the correction only removes an implicit assumption that no longer matches the chosen mechanism, and introduces no new mechanism, file format, or SQL syntax into spec.md itself. No new [NEEDS CLARIFICATION] markers were introduced.
