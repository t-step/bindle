...
* **A published, independently versioned, read-only SQLite export (`src/bindle/symphony_projection.py`'s `publish()`, `contracts/symphony-projection-v1.md`) is Bindle's first externally-supported data interface distinct from its own internal ledger schema.** It is a wholly separate, disposable, regenerated file (`.bindle-work/symphony-projection.sqlite3`, a sibling of the internal `ledger.sqlite3` under the same Git-common-directory resolution) — never a `CREATE VIEW` inside the internal file, and never itself a durable source of truth (`docs/PHILOSOPHY.md` D014/D015; `docs/DATA-OWNERSHIP.md`'s "Durable versus derived"). It contains exactly one row per non-archived `type = 'task'` work item (`WorkLedger.generate_external_projection()`, new, reusing the existing `_STILL_BLOCKING_CONDITION` fragment rather than a second copy of the blocking predicate) — a `type = 'milestone'` row never appears, under any status, claim, or blocking state. Each row carries a stable id, a workspace-name-safe `identifier`, `title`, `description`, a direct `status` string, and a derived `dispatchable` boolean (`type = 'task' AND status = 'open' AND NOT claimed AND NOT blocked` — identical to the ledger's existing "available to start" computation, computed entirely inside Bindle so the external reader never evaluates blocking or claims itself). The file's own `PRAGMA user_version` (currently `1`) versions this contract independently of the internal ledger's `_SCHEMA_VERSION`; an incompatible future shape ships as `contracts/symphony-projection-v2.md`, mirroring the `coordinator-projection.md` → `coordinator-projection-v2.md` precedent 002 already established. The existing internal `generate_projection()`/`ProjectedWorkItem` contract (001, extended by 002) is unchanged and continues to serve its own separate, in-process purpose.
